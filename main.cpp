...
#include <windows.h>
#include <direct.h> // Necesario para crear la carpeta sesiones automáticamente
...
// Escapa comillas simples para que el comando de PowerShell no se rompa con frases
...
private:
    ...
    void actualizarAltura(NodoAVL* n) {
        if (n) n->altura = 1 + max(altura(n->izquierdo), altura(n->derecho));
    }
...
string traducir(const string& texto, const string& lenguajeDestino) {
    ...
    if (curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);  // esto le dice a CURL que ignore la validacion del certificado SSL
        ...
        if (res == CURLE_OK) {
            ...
            if (j.contains("data")) {
                ...
                cout << "\n[DEBUG] Error de Google: " << j.dump(4) << "\n"; 
            }
        } else {
            cout << "\n[DEBUG] Falla de CURL: " << curl_easy_strerror(res) << "\n";
        }
    }
...
void mostrarHistorial(ArbolAVL& arbol) {
    ...
    cout << left << setw(20) << "Palabra/Frase"
                 << setw(25) << "Traduccion"
                 << setw(8)  << "Idioma"
                 << "Busquedas\n";
    cout << string(60, '-') << "\n";
...
int main() {
    _mkdir("sesiones"); // Crea carpeta si no existe
    ...
    do {
        ...
        cout << "  2. Agregar manual (palabra/frase)"<<endl;
        ...
        switch (opcion) {
            case 1: {
                ...
                cout << "TRADUCIR"<<endl;
                cout << "Palabra o frase a traducir: "<<endl;
                cin >> ws; // <--- CAMBIO CLAVE: Limpia basura del buffer correctamente
                getline(cin, palabra); 
            ...
            case 3: {
                ...
                cout << "Frase a eliminar: "<<endl;
                cin >> ws; // <--- CAMBIO CLAVE
                getline(cin, palabra);
                if (historial.eliminar(palabra))
                    cout << "\n[Nodo eliminado del arbol AVL]\n";
                else
                    cout << "\n[Frase no encontrada en el historial]\n";
                pausar();
                break;
            }
        }
